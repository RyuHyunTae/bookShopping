...
<body
	...
	<table class="type05">
		...
		<%
			for (CreditcardVO card : creditcardList) {
		%>
		<tr>
			<td><%=card.getCreditcardNum()%></td>
			<td><%=card.getLicense()%></td>
			<td><%=card.getCreditcardKind()%></td>
			<td><form action="deleteCreditcard.do" method="post">
			<input type="hidden" name="creditcardNum" value="<%=card.getCreditcardNum()%>">
	...
	<button onclick="location.href='insertCreditcard.jsp'" class="btn btn-outline-secondary">카드추가</button>
	</div>
</body>
</html>