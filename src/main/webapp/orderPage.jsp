...
<%
	...
	BookVO oneList = (BookVO) request.getAttribute("oneList");
...
<body
	...
	<h3>구매화면</h3>
	<form action="insertOneOrder.do" method="post">
	<table class="type05">
		<tr>
			<td width="70">사진</td>
			<td width="500">물품명</td>
		...
		<tr>
			<td><img
				src="http://localhost:8080\biz\img\<%=oneList.getBookPicture()%>"
				width="70" height="70"></td>
			<td width="500" cellpadding="5" cellspacing="2"
				style="table-layout: fixed; word-break: break-all;"><%=oneList.getBookName()%></td>
				<%if(grade.equals("a")){
					double a=oneList.getBookPrice()*0.8;
					int b=(int)a;
					sum +=b;
					%>
				<td><%=b%><input type="hidden" name="orderPrice" value="<%=b%>"></td>
				<%} else if(grade.equals("b")){ 
					double a=oneList.getBookPrice()*0.9;
					int b=(int)a;
					sum +=b;
					%>
				<td><%=b%><input type="hidden" name="orderPrice" value="<%=b%>"></td>
				<%} else if(grade.equals("c")){ 
					double a=oneList.getBookPrice();
					int b=(int)a;
					sum +=b;
					%>
				<td><%=b%><input type="hidden" name="orderPrice" value="<%=b%>"></td>
		...
		<%} %>
		<input type="hidden" name="bookNum" value="<%=oneList.getBookNum()%>">
		<input type="hidden" name="bookPicture" value="<%=oneList.getBookPicture()%>">
		<input type="hidden" name="bookName" value="<%=oneList.getBookName()%>">
